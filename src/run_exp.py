...
from src.solver import centralized_solver, centralized_solver_for, centralized_solver_multi_gpu, centralized_solver_watermark
from src.utils import generate_watermark
import logging
import os
import h5py
import numpy as np
    ...
    for K in range(10):
        for file_name in os.listdir(folder_path):
            if not file_name.startswith('.'):
                ...
                # elif params['data'] == "GM":
                #     constraints, header = read_gm(path)
    ...
    for file_name in os.listdir(folder_path):
        if not file_name.startswith('.'):
            ...
            if params['data'] == "uf":
                ...
                log.warning(
                    'Data mode does not exist. Add the data mode. Current version only support uf, stanford, random_reg, hypergraph, arxiv, and NDC.')
            ...
            # maxclique_data.seed(12)
            # # Create a list of all nodes and shuffle for maxclique_data distribution
            # total_nodes = header['num_nodes']
            # all_nodes = list(range(1, total_nodes + 1))
            # maxclique_data.shuffle(all_nodes)

            # # Split nodes randomly among devices
