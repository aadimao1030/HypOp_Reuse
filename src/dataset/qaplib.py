...
class QAPLIB(BaseDataset):
    def __init__(self, sets, cls, fetch_online=False):
        ...
        for inst in self.cls_list:
            ...
            for dat_path in self.qap_path.glob(inst + '*.dat'):
                ...
                if (self.sets == 'test' and prob_size > 100) \
                    or (self.sets == 'train' and prob_size > 200):
                    continue
    ...
    def __fetch_online(self):
        ...
        for name in self.data_list:
            url = 'https://qaplib.mgi.polymtl.ca/' + 'data.d/{}.dat'.format(name)
            dat_content = urllib.request.urlopen('https://qaplib.mgi.polymtl.ca/' + 'data.d/{}.dat'.format(name)).read()
            sln_content = urllib.request.urlopen('https://qaplib.mgi.polymtl.ca/' + 'soln.d/{}.sln'.format(name)).read()
