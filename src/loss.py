...
def loss_maxclique_weighted(probs, C, dct, weights, p=4):
    ...
    x = probs.squeeze()  # 将 probs 转换为一维张量
    loss = -sum(x)  # 目标是最大化团的大小，因此我们要最小化负的团大小

    # 遍历每一条边，并计算缺少的边所带来的惩罚
    for c, w in zip(C, weights):
        temp = (p * w * x[dct[c[0]]] * x[dct[c[1]]])  # 权重加上节点属于团的概率
        loss += temp  # 加入损失，表示团内节点之间的边的权重
